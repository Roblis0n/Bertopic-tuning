# Repository Background and Full-Release Capabilities

This file preserves the detailed capability background that predates
assurance-aware routing. Treat statements about mandatory full reconnaissance,
approval, visualization, and release artifacts as
`publication_release` obligations. Use `SKILL.md` and
`semantic-review-and-cumulative-tuning.md` for the current workflow.
...
See [network-short-text.md](network-short-text.md) for the complete route.
...
See [long-document.md](long-document.md) for the complete route.
...
For metric definitions and calibration rules, see [diversity-evaluation.md](diversity-evaluation.md).
...
See [research-grade-visualization.md](research-grade-visualization.md) for the complete method.
...
The coarse/fine estimate is explicitly `pre_model_hypothesis_not_target_k`; it supports later coverage and missing-theme checks but never forces BERTopic's topic count. See [corpus-theme-reconnaissance.md](corpus-theme-reconnaissance.md).
...
The planner and validator use only the Python standard library. Rendering remains in the fitted BERTopic/Plotly environment and follows [research-grade-visualization.md](research-grade-visualization.md).
...
Custom terms protect tokenizer phrases; they are not passed as a closed `CountVectorizer(vocabulary=...)` allowlist. Review every generated candidate before changing a source table. See [lexicon-management-and-iteration.md](lexicon-management-and-iteration.md).
...
Permanent `topic_uid` values are kept separate from BERTopic's local integer IDs. Each update records continuations, new topics, retirements, merges, splits, unresolved mappings, and the evidence behind those decisions. See [iteration-and-lineage.md](iteration-and-lineage.md).
...
- [corpus-theme-reconnaissance.md](corpus-theme-reconnaissance.md) for the corpus-scale preview, count estimation, and authorization gate;
- [scalable-corpus-reading.md](scalable-corpus-reading.md) for large/multi-file extraction, adaptive selection, full-text escalation, holdout auditing and residual-risk reporting;
- [academic-evidence.md](academic-evidence.md) for the evidence map and source-to-decision boundaries;
- [web-research-protocol.md](web-research-protocol.md) when claims depend on current papers, APIs, package behavior, or model availability;
- [study-contract-and-reporting.md](study-contract-and-reporting.md) for required artifacts and reporting standards;
- [research-grade-visualization.md](research-grade-visualization.md) for the layered core/conditional figure system and artifact contract;
- [bertopic-implementation.md](bertopic-implementation.md) when implementing or reviewing BERTopic code.
