...
单一优化目标。完成修改后运行全部测试和技能验证；若修改了技能功能文件，
...
`assets/` 中已经包含研究合同、语料画像、实验登记、候选指标、模型选择、主题目录、人工审计、遗漏主题审计、主题对审计、谱系、证据日志和决策报告模板。
